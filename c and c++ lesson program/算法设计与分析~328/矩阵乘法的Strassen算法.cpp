...
void Strassen_class<T>::FillMatrix( T** MatrixA, T** MatrixB, int length)
{
    ...
    for(int row = 0; row<length; row++)
    {
        for(int column = 0; column<length; column++)
        {
            ...
            MatrixB[row][column] = (MatrixA[row][column] = rand() %max);
...
{
	Strassen_class<unsigned char> stra;//定义Strassen_class类对象
    int MatrixSize = 0;

    unsigned char** MatrixA;    //存放矩阵A
    unsigned char** MatrixB;    //存放矩阵B
    unsigned char** MatrixC;    //存放结果矩阵
    ...
    //cout<<"\n请输入矩阵大小(必须是2的幂指数值(例如:32,64,512,..): ";cin>>MatrixSize;
    ...
    //申请内存
    MatrixA = new unsigned char *[MatrixSize];
    MatrixB = new unsigned char *[MatrixSize];
    MatrixC = new unsigned char *[MatrixSize];

    for (int i = 0; i < MatrixSize; i++)
    {
        MatrixA[i] = new unsigned char [MatrixSize];
        MatrixB[i] = new unsigned char [MatrixSize];
        MatrixC[i] = new unsigned char [MatrixSize];
    }
