...
void jsonxx::assertion( const char *file, int line, const char *expression, bool result ) {
    if( !result ) {
		...
		debugPrintWithTag("guan", "[JSONXX] error: expression '%s' failed at %s:%d -> ", expression, file, line );
        assert( 0 );
    }
}
...
bool Object::parse(std::istream& input, Object& object) {
    ...
    do {
        ...
        auto it = object.value_map_.find(key);
        if (it == object.value_map_.end())
            object.value_map_.insert(std::make_pair(key, v));
        else
            it->second = v;
